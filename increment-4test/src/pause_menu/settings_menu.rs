use bevy::{ecs::{relationship::RelatedSpawner, spawn::SpawnWith}, prelude::*};

use crate::{directional::{DirectionalNavigator, Directions}, dirq, state::PauseState};

use super::{shared_widgets::{hover_observer, out_observer, pause_menu_button_widget, pressed_observer}};


#[derive(Component, Debug, Clone, Copy, PartialEq)]
pub enum SettingsButtons{
    Keybinds,
    PlaceHolder,
    Return
}


#[derive(Component)]
pub struct PauseMenuSettingsUITag;

pub fn spawn_pause_menu_settings(
    mut commands: Commands,
){
    let mut entities: Vec<(Entity, Directions)> = vec![]; 
    commands
        .spawn((
            Node {
                width: Val::Percent(100.),
                height: Val::Percent(100.),
                flex_direction: FlexDirection::Column,
                align_items: AlignItems::Center,
                justify_content: JustifyContent::SpaceEvenly,
                ..Default::default()
            },
            DirectionalNavigator::new(
                [
                    [
                        (SettingsButtons::Keybinds, dirq!(South))
                    ],
                    [
                        (SettingsButtons::PlaceHolder, dirq!(South, North))
                    ],
                    [
                        (SettingsButtons::Return, dirq!(North))
                    ]
                ]
            ),
            PauseMenuSettingsUITag,
            children![ 
                (
                    Node {
                        width: Val::Percent(30.),
                        height: Val::Percent(30.),
                        flex_direction: FlexDirection::Column,
                        justify_content: JustifyContent::SpaceEvenly,
                        ..Default::default()
                    },
                    Children::spawn(SpawnWith(move |parent: &mut RelatedSpawner<ChildOf>|{
                        spawn_pause_menu_settings_buttons(parent, &mut entities);
                    }))
                )
            ]
        ))
    ;
}




fn spawn_pause_menu_settings_buttons(parent: &mut RelatedSpawner<ChildOf>, entities: &mut Vec<(Entity, Directions)>){
    macro_rules! spawn {
        ($text:tt, $onclick:expr, $( $dir:ident ),*) => {
            entities.push(
                (
                    parent.spawn(pause_menu_button_widget($text))
                        .observe($onclick)
                        .observe(hover_observer)  
                        .observe(out_observer)
                        .observe(pressed_observer).id(),
                    $crate::directional::Directions::new(&[$(
                        bevy::math::CompassQuadrant::$dir,
                    )*])
                )
            )
        };
    }
    spawn!("Keybinds", onclick_keybinds, South);
    spawn!("Fun Grid", onclick_fun_grid, South, North);
    spawn!("Return", onclick_return, North);
}


fn onclick_keybinds(_trigger: Trigger<Pointer<Released>>){
    info!("Let's see that functionnality in a future increment.");
}

fn onclick_fun_grid(_trigger: Trigger<Pointer<Released>>, mut next_state: ResMut<NextState<PauseState>> ){
    next_state.set(PauseState::PauseMenuFunGrid);
}

fn onclick_return(_trigger: Trigger<Pointer<Released>>, mut next_state: ResMut<NextState<PauseState>> ){
    next_state.set(PauseState::PauseMenu);
}